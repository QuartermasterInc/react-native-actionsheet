...
class ActionSheet extends Component {
  ...
  _createButton (option, fontColor, index, style) {
    let titleNode = (
    ...
    if (option.icon) {
      iconNode = (
        <Text style={[btnStyle.icon, { color: fontColor }]}>{option.icon}</Text>
      )
    }
    return (
      <TouchableHighlight
  ...
  _renderOptions () {
    ...
    return options.map((option, index) => {
      ...
      return index === cancelButtonIndex
        ? null
        : this._createButton(option, fontColor, index)
    })
  }
...
ActionSheet.propTypes = {
  ...
  options: PropTypes.arrayOf(
    (propVal, key, componentName, location, propFullName) => {
      if (
